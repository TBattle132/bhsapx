...
<head>
  <meta charset="utf-8">
  <title>BHS • Edit Access ID</title>
...
<body>
  <div class="wrap">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:18px">
      <h2 style="margin:0"><span class="badge">BHS</span> Edit Access ID</h2>
      <div>
    ...
    <form method="POST" action="{{ route('access.update', $item->id) }}">
      @csrf @method('PUT')
      ...
      <div class="row">
        <div class="field">
          <label>Token</label>
          <input name="token" value="{{ old('token', $item->token) }}" required>
          @error('token') <div class="err">{{ $message }}</div> @enderror
        </div>
        <div class="field">
          <label>TX Allowed</label>
          <select name="tx_allowed">
            <option value="0" @selected(old('tx_allowed', $item->tx_allowed ? '1':'0')==='0')>No</option>
            <option value="1" @selected(old('tx_allowed', $item->tx_allowed ? '1':'0')==='1')>Yes</option>
          </select>
        ...
        <div class="field">
          ...
          <select name="codeplug_id">
            ...
            @foreach ($codeplugs as $cp)
              <option value="{{ $cp->id }}" @selected(old('codeplug_id', $item->codeplug_id)==$cp->id)>{{ $cp->name }}</option>
            @endforeach
          </select>
      ...
      <button class="btn" type="submit" style="background:#0E2E27;border-color:#0BAE87">Save</button>
    </form>
  </div>
</body>
</html>
